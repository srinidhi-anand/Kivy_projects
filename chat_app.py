...
class ConnectPage(GridLayout):
    def __init__(self, **kwargs):
        ...
        if os.path.isfile("prev_details.txt"):
            with open("prev_details.txt", "r") as f:
                d = f.read().split(",")
                prev_ip, prev_port, prev_username = d[0], d[1], d[2]
        else:
    ...
    def connect(self, _):
        port, ip, username = int(self.port.text), self.ip.text, self.username.text
        print(type(ip))
        print(username)
...
class ChatPage(GridLayout):
    def __init__(self, **kwargs):
        ...
        self.cols = 1
        self.rows = 2
        # self.add_widget(Label(text="Hey at least worked up to this point!!!"))
        ...
        socket_client.start_listening(self.incoming_messgae, show_error)
    ...
    def adjust_fields(self, *_):
        ...
        self.new_message_width = new_width

        Clock.schedule_once(self.history.update_chat_history_layout, 0.01)

    def on_key_down(self, instance, keydown, keycode, text, modifiers):
        if keycode == 40:
